...
public class ImageFilters {
    ...
    public Bitmap applyGammaEffect(Bitmap src, double red, double green, double blue) {
        ...
        for (int i = 0; i < MAX_SIZE; ++i) {
            gammaR[i] = (int) Math.min(MAX_VALUE_INT, (int) ((MAX_VALUE_DBL *
                    Math.pow(i / MAX_VALUE_DBL, REVERSE / red)) + 0.5));
            gammaG[i] = (int) Math.min(MAX_VALUE_INT, (int) ((MAX_VALUE_DBL *
                    Math.pow(i / MAX_VALUE_DBL, REVERSE / green)) + 0.5));
            gammaB[i] = (int) Math.min(MAX_VALUE_INT, (int) ((MAX_VALUE_DBL *
                    Math.pow(i / MAX_VALUE_DBL, REVERSE / blue)) + 0.5));
        }
    ...
    public Bitmap applyGaussianBlurEffect(Bitmap src) {
        ...
        ConvolMatrix convMatrix = new ConvolMatrix(3);
        convMatrix.applyConfig(GaussianBlurConfig);
        convMatrix.Factor = 16;
        convMatrix.Offset = 0;
        return ConvolMatrix.getResult(src, convMatrix);
    }

    public Bitmap applySharpenEffect(Bitmap src, double weight) {
        double[][] SharpConfig =
                new double[][]{{0, -2, 0}, {-2, weight, -2}, {0, -2, 0}};
        ConvolMatrix convMatrix = new ConvolMatrix(3);
    ...
    public Bitmap applyWaterMarkEffect(Bitmap src, String watermark, int x, int y,
    ...
    public Bitmap applyFleaEffect(Bitmap source) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                // get random color
                int randColor =
                        Color.rgb(random.nextInt(COLOR_MAX), random.nextInt(COLOR_MAX),
    ...
    public Bitmap applyReflection(Bitmap originalImage) {
        ...
        Bitmap reflectionImage =
                Bitmap.createBitmap(originalImage, 0, height / 2, width, height / 2,
                        matrix, false);
